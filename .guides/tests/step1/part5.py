...
def check_step5(*files):
    ...
    if not result[0]:
        result = verifier.grade_mod_structure(files[0],1)
    if not result[0]:
        result = verifier.grade_app_structure(files[1],1)
    if not result[0]:
        result = verifier.grade_test_structure(files[2],1)
    if not result[0]:
        result = verifier.grade_proc_headers(files[2],0)
    if not result[0]:
...
if __name__ == '__main__':
    sys.exit(check_step5('currency.py','exchangeit.py','testcurrency.py'))
